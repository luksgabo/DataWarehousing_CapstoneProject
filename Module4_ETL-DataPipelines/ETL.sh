...
timestamp_then=$(date -d "$timestamp_now 5 hours ago" "+%Y-%m-%d %H:%M:%S")
...
mysql -h mysql -P 3306 -u root --password=lGmNDlKAu7RrXN3BJHQ7VIzG --database=sales --execute="select * from sales_data where timestamp > '$timestamp_then' " --batch --silent > /home/project/sales.csv
...
export PGPASSWORD=6VX5RRzlx6NNsVDhZej8TdUA;
...
psql --username=postgres --host=postgres --dbname=sales_new -c "INSERT INTO DimDate(dateid,day,month,year) SELECT dateid, to_char(timestamp, 'day') as day, to_char(timestamp, 'month') as month, to_char(timestamp, 'YYYY') as year from sales_data;"

## Write your code here to load the FactSales table from the data present in sales_data table
psql --username=postgres --host=postgres --dbname=sales_new -c "INSERT INTO FactSales(rowid, product_id, customer_id, price, total_price) SELECT rowid, product_id, customer_id, price, (price*quantity) as total_price from sales_data;"

## Write your code here to export DimDate table to a csv
psql --username=postgres --host=postgres --dbname=sales_new -c "\copy (SELECT * FROM DimDate) TO '/home/project/DimDate.csv' WITH (FORMAT csv, HEADER true);" 

## Write your code here to export FactSales table to a csv
psql --username=postgres --host=postgres --dbname=sales_new -c "\copy (SELECT * FROM FactSales) TO '/home/project/FactSales.csv' WITH (FORMAT csv, HEADER true);" 
